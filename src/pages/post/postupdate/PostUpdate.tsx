...
import ImageUploadStep from './components/ImageUploadStepUpdate';
import SubscriptionSelector from './components/SubscriptionSelectorUpdate';
import { formPostNew } from '../../../stores/FormPostNew';
import { PropertiesFilter } from '../../../types/PropertiesFilter';
import { Pageable } from '../../../types/Pageable';
import AddressModalUpdate from './components/AddressModalUpdate';
import PropertyPreviewModalUpdate from './components/PropertyPreviewModalUpdate';
import LocationSelectModalUpdate from '../postupdate/components/LocaltionsModalUpdate';
...
const PostNew = () => {
    ...
    const onFinish = (values: any) => {
        ...
        if (newPostStore.currentStepUpdate < steps.length - 1) {
            newPostStore.setCurrentStepUpdate(newPostStore.currentStepUpdate + 1);
        }
    };
    ...
    useEffect(() => {
        const fetchPostDetail = async () => {
            if (id) {
                ...
                newPostStore.setPostUpdateDetail(res);
                setSelectedType(res.demand);
                form.setFieldsValue({
                    residentialPropertyId: res.residentialProperty?.id,
                    demand: res.demand,
                    arena: res.arena,
                    price: res.price,
                    unit: res.unit,
                    propertyLegalDocumentId: res.propertyLegalDocument?.id,
                    interiorId: res.interior?.id,
                    numberOfBedrooms: res.numberOfBedrooms,
                    numberOfBathrooms: res.numberOfBathrooms,
                    houseDirectionId: res.houseDirection?.id,
                    balconyDirectionId: res.balconyDirection?.id,
                ...
                setDescriptionLength(res.description.length);
                setTitleLength(res.title.length);
                newPostStore.setStressTotal({
                    street: res.street,
                })
            }
        };
        fetchPostDetail();
    }, [id, form, newPostStore]);
    ...
    return (
        <div className="min-vh-100 bg-light">
            ...
            <div className="w-100 border-bottom bg-white">
                <div className="container px-4 py-4 d-flex justify-content-between align-items-center">
                    ...
                    <div className="d-flex gap-3">
                        {newPostStore.currentStepUpdate === 2 ? (
            ...
            <div className="container px-4 py-5">
                <Steps
                    current={newPostStore.currentStepUpdate}
                ...
                <Form
                    ...
                    initialValues={{
                        demand: newPostStore.postUpdateDetail?.demand || 'SELL',
                        residentialPropertyId: newPostStore.postUpdateDetail?.residentialProperty?.id,
                    }}
                >
                    {newPostStore.currentStepUpdate === 0 && (
                        <>
                    ...
                    {newPostStore.currentStepUpdate === 1 && (
            ...
            <AddressModalUpdate />
            <LocationSelectModalUpdate />
            <PropertyPreviewModalUpdate />
        </div>
    )
}
