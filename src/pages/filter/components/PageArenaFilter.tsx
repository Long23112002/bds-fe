...
const PageArenaFilter = () => {
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            if (listRef.current && !listRef.current.contains(event.target as Node)) {
                pageFilterStore.setIsOpenArenaFilter(false);
            }
        };
    ...
    const handleApply = () => {
        ...
        pageFilterStore.setAreaSearchValue({
            min: sliderValue[0],
            max: sliderValue[1]
        })
    ...
    return (
        <Card ref={listRef} style={{
            display: pageFilterStore.isOpenArenaFilter ? 'block' : 'none',
            width: "340px", height: '460px', overflow: 'auto',
            position: 'absolute', top: '60px', left: '1275px'
        }}>
...
export default observer(PageArenaFilter)
